...
headlabelfont = ("Noto Sans CJK TC", 15, 'bold','italic')
labelfont = ('Garamond', 14,'italic')
...
connector.execute(
"CREATE TABLE IF NOT EXISTS SCHOOL_MANAGEMENT (STUDENT_ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, NAME TEXT, EMAIL TEXT, PHONE_NO TEXT, GENDER TEXT, STREAM TEXT)"
)
...
def update_record():
    ...
    if not tree.selection():
        ...
        if not name or not email or not contact or not gender or not stream:
            ...
            try:
                connector.execute(
                    'UPDATE SCHOOL_MANAGEMENT SET NAME=?, EMAIL=?, PHONE_NO=?, GENDER=?, STREAM=? WHERE STUDENT_ID=?', 
                    (name, email, contact, gender, stream, selection[0])
                )
                connector.commit()
                mb.showinfo('Record updated', f"Record of {name} was updated successfully")
                reset_fields()
                display_records()
            except:
                mb.showerror('Wrong type', 'The type of the values entered is not accurate.')
...
lf_bg = 'MediumSpringGreen' # bg color
cf_bg = 'PaleGreen' 
